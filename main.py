...
config_path = Path('webapp.env').absolute()
config = dotenv_values(dotenv_path=config_path)

if not config:
    print(f"Error: Failed to load configuration from {config_path}")
    print("Please ensure the file exists and contains the required variables.")
    sys.exit(1)

required_keys = ['issuer_url', 'client_id', 'client_secret', 'idc_provider_apl_arn', 'qb_sts_role', 'qb_apl_id', 'app_domain']
missing_keys = [key for key in required_keys if key not in config]

if missing_keys:
    print(f"Error: The following required configuration keys are missing: {', '.join(missing_keys)}")
    print(f"Please check your {config_path} file and ensure all required variables are set.")
    sys.exit(1)
